...
public class HiveCalcitePlanner extends SemanticAnalyzer {
  ...
  public HiveCalcitePlanner(QueryState queryState) throws SemanticException {
    super(queryState);
  ...
  /**
   * This method is useful if we want to obtain the logical plan after being parsed and optimized by
   * Calcite.
   *
...
//    }
  ...
  private static RelOptPlanner createPlanner(HiveConf conf, Set<RelNode> corrScalarRexSQWithAgg,
      Set<RelNode> scalarAggNoGbyNoWin) {
    final Double maxSplitSize = (double) HiveConf.getLongVar(conf,
        HiveConf.ConfVars.MAPREDMAXSPLITSIZE);
    final Double maxMemory = (double) HiveConf.getLongVar(conf,
        HiveConf.ConfVars.HIVECONVERTJOINNOCONDITIONALTASKTHRESHOLD);
    HiveAlgorithmsConf algorithmsConf = new HiveAlgorithmsConf(maxSplitSize, maxMemory);
    HiveRulesRegistry registry = new HiveRulesRegistry();
    Properties calciteConfigProperties = new Properties();
    calciteConfigProperties.setProperty(CalciteConnectionProperty.TIME_ZONE.camelName(),
        conf.getLocalTimeZone().getId());
    calciteConfigProperties.setProperty(
        CalciteConnectionProperty.MATERIALIZATIONS_ENABLED.camelName(), Boolean.FALSE.toString());
    CalciteConnectionConfig calciteConfig = new CalciteConnectionConfigImpl(
        calciteConfigProperties);
          ...
          if (this.conf.getBoolVar(HiveConf.ConfVars.HIVE_CBO_RETPATH_HIVEOP)) {
            ...
            if (cboCtx.type == PreCboCtx.Type.VIEW && !materializedView) {
              ...
              String originalText = ctx.getTokenRewriteStream()
                  .toString(cboCtx.nodeOfInterest.getTokenStartIndex(),
          ...
          if (isMissingStats) {
            ...
            if (e instanceof CalciteSemanticException) {
              ...
              if (unsupportedFeature != null) {
                this.ctx.setCboInfo(
                    "Plan not optimized by CBO due to missing feature [" + unsupportedFeature
                        + "].");
              } else {
  ...
  private ASTNode rewriteASTForMultiInsert(ASTNode query, ASTNode nodeOfInterest) {
    ...
    ASTNode destination = new ASTNode(
    ...
    for (Collection<Object> selectIdentifier : aliasNodes.asMap().values()) {
      ...
      ASTNode colExpr = new ASTNode(
          new CommonToken(HiveParser.TOK_TABLE_OR_COL, "TOK_TABLE_OR_COL"));
   ...
   * @return boolean
   * <p>
   * Assumption:<br> If top level QB is query then everything below it must also be Query.
   */
  void checkCanCBOHandleAst(ASTNode ast, QB qb, PreCboCtx cboCtx) throws SemanticException {
    ...
    boolean isSupportedType = (qb.getIsQuery()) || qb.isCTAS() || qb.isMaterializedView()
    ...
    if (!result) {
      if (needToLogMessage) {
        ...
        STATIC_LOG.info(
            "Not invoking CBO because the statement " + msg.substring(0, msg.length() - 2));
        throw new SemanticException("CBO can't handle ast:" + msg);
      }
    }
   ...
   * <p>
   * Assumption:<br> 1. If top level QB is query then everything below it must also be Query<br> 2.
   * Nested Subquery will return false for qbToChk.getIsQuery()
   */
  static String canHandleQbForCbo(QueryProperties queryProperties, HiveConf conf,
    ...
    if (!queryProperties.hasClusterBy() && !queryProperties.hasDistributeBy()
        ...
        && !queryProperties.usesScript() && queryProperties.isCBOSupportedLateralViews()) {
      // Ok to run CBO.
      return null;
    }
...
//    }
  ...
  boolean isCBOSupportedLateralView(ASTNode lateralView) {
    ...
    if (innerFunc.getToken().getType() != HiveParser.TOK_FUNCTION || innerFunc.getChild(0).getText()
        .compareToIgnoreCase(GenericUDFArray.class.getAnnotation(Description.class).name()) != 0) {
      return false;
    }
    return true;
  }
  ...
  Table materializeCTE(String cteName, CTEClause cte) throws HiveException {
    ...
    ASTNode temporary = new ASTNode(
        new ClassicToken(HiveParser.KW_TEMPORARY, MATERIALIZATION_MARKER));
  ...
  private void fixUpASTAggregateIncrementalRebuild(ASTNode newAST) throws SemanticException {
    ...
    ASTNode updateNode = new ASTSearcher().simpleBreadthFirstSearch(newAST, HiveParser.TOK_QUERY,
        HiveParser.TOK_INSERT);
    ASTNode destinationNode = (ASTNode) updateNode.getChild(0);
    ASTNode newInsertInto = (ASTNode) ParseDriver.adaptor.create(HiveParser.TOK_INSERT_INTO,
        "TOK_INSERT_INTO");
    ...
    ASTNode materializationNode = new ASTSearcher().simpleBreadthFirstSearch(newInsertInto,
        HiveParser.TOK_INSERT_INTO, HiveParser.TOK_TAB, HiveParser.TOK_TABNAME);
    ...
    //          cmv_mat_view
    //       ROW__ID
    ASTNode subqueryNodeInputROJ = new ASTSearcher().simpleBreadthFirstSearch(newAST,
        HiveParser.TOK_QUERY, HiveParser.TOK_FROM, HiveParser.TOK_RIGHTOUTERJOIN,
        HiveParser.TOK_SUBQUERY);
    ASTNode selectNodeInputROJ = new ASTSearcher().simpleBreadthFirstSearch(subqueryNodeInputROJ,
        HiveParser.TOK_SUBQUERY, HiveParser.TOK_QUERY, HiveParser.TOK_INSERT,
        HiveParser.TOK_SELECT);
    ASTNode selectExprNodeInputROJ = (ASTNode) ParseDriver.adaptor.create(HiveParser.TOK_SELEXPR,
        "TOK_SELEXPR");
    ASTNode dotNodeInputROJ = (ASTNode) ParseDriver.adaptor.create(HiveParser.DOT, ".");
    ASTNode columnTokNodeInputROJ = (ASTNode) ParseDriver.adaptor.create(
        HiveParser.TOK_TABLE_OR_COL, "TOK_TABLE_OR_COL");
    ASTNode tableNameNodeInputROJ = (ASTNode) ParseDriver.adaptor.create(HiveParser.Identifier,
        Warehouse.getQualifiedName(materializationNode.getChild(0).getText(),
            materializationNode.getChild(1).getText()));
    ASTNode rowIdNodeInputROJ = (ASTNode) ParseDriver.adaptor.create(HiveParser.Identifier,
    ...
    ASTNode tableNameNodeInUpdate = (ASTNode) ParseDriver.adaptor.dupNode(
    ...
    //                     cmv_basetable_2
    //                  ROW__ID
    ASTNode sortExprNode = (ASTNode) ParseDriver.adaptor.create(HiveParser.TOK_SORTBY,
        "TOK_SORTBY");
    ASTNode orderExprNode = (ASTNode) ParseDriver.adaptor.create(HiveParser.TOK_TABSORTCOLNAMEASC,
        "TOK_TABSORTCOLNAMEASC");
    ASTNode nullsOrderExprNode = (ASTNode) ParseDriver.adaptor.create(HiveParser.TOK_NULLS_FIRST,
        "TOK_NULLS_FIRST");
  ...
  private void fixUpASTNoAggregateIncrementalRebuild(ASTNode newAST) throws SemanticException {
    ...
    ASTNode newChild = (ASTNode) ParseDriver.adaptor.create(HiveParser.TOK_INSERT_INTO,
        "TOK_INSERT_INTO");
  ...
  /**
   * The context that doPhase1 uses to populate information pertaining to CBO (currently, this is
   * used for CTAS and insert-as-select).
   */
  ...
  ASTNode reAnalyzeViewAfterCbo(ASTNode newAst) throws SemanticException {
    ...
    if (newAst == null) {
      LOG.error(
          "analyzeCreateTable failed to initialize materialized view after CBO;" + " new ast is "
              + getAST().dump());
  ...
  public static class ASTSearcher {
     ...
     * multiple instances of some token, of which only one matches, it is not guaranteed to be
     * found. We use this for simple things. Not thread-safe - reuses searchQueue.
     */
  ...
  RelNode logicalPlan() throws SemanticException {
    ...
    calcitePlannerAction = new CalcitePlannerAction(prunedPartitions,
        ctx.getOpContext().getColStatsCache(), this.columnAccessInfo);

    try {
      optimizedOptiqPlan = Frameworks.withPlanner(calcitePlannerAction,
  ...
  Operator getOptimizedHiveOPDag() throws SemanticException {
    ...
    RelNode modifiedOptimizedOptiqPlan = PlanModifierForReturnPath.convertOpTree(optimizedOptiqPlan,
        resultSchema, this.getQB().getTableDesc() != null);

    LOG.debug(
        "Translating the following plan:\n" + RelOptUtil.toString(modifiedOptimizedOptiqPlan));
    Operator<?> hiveRoot = new HiveOpConverter(this, conf, unparseTranslator, topOps).convert(
        modifiedOptimizedOptiqPlan);
  ...
  Operator<?> handleInsertStatement(String dest, Operator<?> input, RowResolver inputRR, QB qb)
    ...
    Operator<?> output = putOpInsertMap(
        OperatorFactory.getAndMakeChild(new SelectDesc(colList, columnNames),
  ...
  private void rethrowCalciteException(Exception e) throws SemanticException {
    Throwable first = (semanticException != null) ? semanticException
        : e, current = first, cause = current.getCause();
    while (cause != null) {
      ...
      if (current == first && causeOfCause == null && isUselessCause(first)) {
        ...
        first = cause;
        break;
      } else if (causeOfCause != null && isUselessCause(cause) && ExceptionHelper.resetCause(
          current, causeOfCause)) {
  ...
  private static class ExceptionHelper {
    ...
    private static final Field CAUSE_FIELD = getField(Throwable.class,
        "cause"), TARGET_FIELD = getField(InvocationTargetException.class,
        "target"), MESSAGE_FIELD = getField(Throwable.class, "detailMessage");
  ...
  private RowResolver genRowResolver(Operator op, QB qb) {
    ...
    String subqAlias =
        (qb.getAliases().size() == 1 && qb.getSubqAliases().size() == 1) ? qb.getAliases().get(0)
            : null;
  ...
  private enum ExtendedCBOProfile {
    JOIN_REORDERING, WINDOWING_POSTPROCESSING;
  }
  ...
  private class CalcitePlannerAction implements Frameworks.PlannerAction<RelNode> {
    ...
    CalcitePlannerAction(Map<String, PrunedPartitionList> partitionCache,
        Map<String, ColumnStatsList> colStatsCache, ColumnAccessInfo columnAccessInfo) {
    ...
    public RelNode apply(RelOptCluster cluster, RelOptSchema relOptSchema, SchemaPlus rootSchema) {
      ...
      try {
        ...
        resultSchema = SemanticAnalyzer.convertRowSchemaToResultSetSchema(
            relToHiveRR.get(calciteGenPlan), getQB().isView() ? false : HiveConf.getBoolVar(conf,
      ...
      if (conf.getBoolVar(ConfVars.HIVE_MATERIALIZED_VIEW_ENABLE_AUTO_REWRITING)
          ...
          && !getQB().isCTAS()) {
        calcitePreCboPlan = applyMaterializedViewRewriting(planner, calcitePreCboPlan,
            mdProvider.getMetadataProvider(), executorProvider);
      }
      ...
      if (profilesCBO.contains(ExtendedCBOProfile.JOIN_REORDERING)) {
        ...
        try {
          ...
          RelTraitSet desiredTraits = optCluster.traitSetOf(HiveRelNode.CONVENTION,
              RelCollations.EMPTY);
          ...
          optCluster.setMetadataProvider(
      ...
      calciteOptimizedPlan = hepPlan(calciteOptimizedPlan, false, mdProvider.getMetadataProvider(),
          null, HepMatchOrder.BOTTOM_UP, ProjectRemoveRule.INSTANCE, HiveUnionMergeRule.INSTANCE,
          HiveAggregateProjectMergeRule.INSTANCE, HiveProjectMergeRule.INSTANCE_NO_FORCE,
          HiveJoinCommuteRule.INSTANCE);
      perfLogger.PerfLogEnd(this.getClass().getName(), PerfLogger.OPTIMIZER,
          "Calcite: Optimizations without stats 1");
      ...
      if (conf.getBoolVar(ConfVars.AGGR_JOIN_TRANSPOSE)) {
        ...
        try {
          calciteOptimizedPlan = hepPlan(calciteOptimizedPlan, false,
              mdProvider.getMetadataProvider(), null, HepMatchOrder.BOTTOM_UP,
              HiveAggregateJoinTransposeRule.INSTANCE);
        } catch (Exception e) {
          boolean isMissingStats = noColsMissingStats.get() > 0;
          if (isMissingStats) {
            LOG.warn(
        ...
        perfLogger.PerfLogEnd(this.getClass().getName(), PerfLogger.OPTIMIZER,
            "Calcite: Aggregate join transpose");
      }
      ...
      if (conf.getBoolVar(ConfVars.SEMIJOIN_CONVERSION)) {
        perfLogger.PerfLogBegin(this.getClass().getName(), PerfLogger.OPTIMIZER);
        calciteOptimizedPlan = hepPlan(calciteOptimizedPlan, false,
            mdProvider.getMetadataProvider(), null, HiveSemiJoinRule.INSTANCE_PROJECT,
      ...
      perfLogger.PerfLogBegin(this.getClass().getName(), PerfLogger.OPTIMIZER);
      calciteOptimizedPlan = hepPlan(calciteOptimizedPlan, false, mdProvider.getMetadataProvider(),
          null, HiveRemoveGBYSemiJoinRule.INSTANCE);
      perfLogger.PerfLogEnd(this.getClass().getName(), PerfLogger.OPTIMIZER,
      ...
      if (conf.getBoolVar(ConfVars.HIVE_REMOVE_SQ_COUNT_CHECK)) {
        perfLogger.PerfLogBegin(this.getClass().getName(), PerfLogger.OPTIMIZER);
        calciteOptimizedPlan = hepPlan(calciteOptimizedPlan, false,
            mdProvider.getMetadataProvider(), null, HiveRemoveSqCountCheck.INSTANCE);
        perfLogger.PerfLogEnd(this.getClass().getName(), PerfLogger.OPTIMIZER,
      ...
      perfLogger.PerfLogBegin(this.getClass().getName(), PerfLogger.OPTIMIZER);
      calciteOptimizedPlan = hepPlan(calciteOptimizedPlan, false, mdProvider.getMetadataProvider(),
          null, HepMatchOrder.BOTTOM_UP, HiveDruidRules.FILTER,
          HiveDruidRules.PROJECT_FILTER_TRANSPOSE, HiveDruidRules.AGGREGATE_FILTER_TRANSPOSE,
          HiveDruidRules.AGGREGATE_PROJECT, HiveDruidRules.PROJECT,
          HiveDruidRules.EXPAND_SINGLE_DISTINCT_AGGREGATES_DRUID_RULE, HiveDruidRules.AGGREGATE,
          HiveDruidRules.POST_AGGREGATION_PROJECT, HiveDruidRules.FILTER_AGGREGATE_TRANSPOSE,
          HiveDruidRules.FILTER_PROJECT_TRANSPOSE, HiveDruidRules.HAVING_FILTER_RULE,
          HiveDruidRules.SORT_PROJECT_TRANSPOSE, HiveDruidRules.SORT,
      ...
      calciteOptimizedPlan = hepPlan(calciteOptimizedPlan, true, mdProvider.getMetadataProvider(),
          null, HepMatchOrder.TOP_DOWN, JDBCExtractJoinFilterRule.INSTANCE,
          JDBCAbstractSplitFilterRule.SPLIT_FILTER_ABOVE_JOIN,
          JDBCAbstractSplitFilterRule.SPLIT_FILTER_ABOVE_CONVERTER, JDBCFilterJoinRule.INSTANCE,
      ...
      if (HiveConf.getBoolVar(conf, ConfVars.HIVE_CBO_RETPATH_HIVEOP)) {
        ...
        calciteOptimizedPlan = hepPlan(calciteOptimizedPlan, true, mdProvider.getMetadataProvider(),
            null, HepMatchOrder.BOTTOM_UP, HiveJoinProjectTransposeRule.BOTH_PROJECT_INCLUDE_OUTER,
        ...
        calciteOptimizedPlan = hepPlan(calciteOptimizedPlan, true, mdProvider.getMetadataProvider(),
            null, HiveFilterProjectTSTransposeRule.INSTANCE,
        ...
        perfLogger.PerfLogEnd(this.getClass().getName(), PerfLogger.OPTIMIZER,
            "Calcite: Translation from Calcite tree to Hive tree");
      }
    ...
    private RelNode applyPreJoinOrderingTransforms(RelNode basePlan, RelMetadataProvider mdProvider,
      ...
      if (!conf.getVar(HiveConf.ConfVars.HIVE_EXECUTION_ENGINE).equals("mr") && conf.getBoolVar(
          HiveConf.ConfVars.HIVEOPTIMIZEDISTINCTREWRITE)) {
        perfLogger.PerfLogBegin(this.getClass().getName(), PerfLogger.OPTIMIZER);
    ...
    private RelNode applyMaterializedViewRewriting(RelOptPlanner planner, RelNode basePlan,
      ...
      try {
        if (mvRebuildMode != MaterializationRebuildMode.NONE) {
          ...
          materializations = Hive.get()
              .getValidMaterializedView(mvRebuildDbName, mvRebuildName, getTablesUsed(basePlan),
                  true);
        } else {
          ...
          materializations = Hive.get()
              .getAllValidMaterializedViews(getTablesUsed(basePlan), false);
        }
              ...
              private RelNode copyNodeScan(RelNode scan) {
                ...
                if (scan instanceof DruidQuery) {
                  ...
                  newScan = DruidQuery.create(optCluster,
                      optCluster.traitSetOf(BindableConvention.INSTANCE), scan.getTable(),
                      dq.getDruidTable(), ImmutableList.<RelNode>of(dq.getTableScan()));
                } else {
          ...
          if (mvRebuildMode == MaterializationRebuildMode.INSERT_OVERWRITE_REBUILD
            ...
            if (visitor.isRewritingAllowed()) {
              ...
              if (visitor.isContainsAggregate()) {
                basePlan = hepPlan(basePlan, false, mdProvider, null, HepMatchOrder.TOP_DOWN,
      ...
      new RelVisitor() {
        ...
        public void visit(RelNode node, int ordinal, RelNode parent) {
          if (node instanceof TableScan) {
            ...
            tablesUsed.add(
                ((RelOptHiveTable) ts.getTable()).getHiveTableMD().getFullyQualifiedName());
          }
    ...
    private RelNode hepPlan(RelNode basePlan, boolean followPlanChanges,
      ...
      RelMetadataProvider chainedProvider = ChainedRelMetadataProvider.of(list);
      basePlan.getCluster()
          .setMetadataProvider(new CachingRelMetadataProvider(chainedProvider, planner));
    ...
    @SuppressWarnings("nls")
    private RelNode genSetOpLogicalPlan(Opcode opcode, String alias, String leftalias,
        RelNode leftRel, String rightalias, RelNode rightRel) throws SemanticException {
      ...
      ASTNode tabref = getQB().getAliases().isEmpty() ? null
          : getQB().getParseInfo().getSrcForAlias(getQB().getAliases().get(0));
      ...
      while (lIter.hasNext()) {
        ...
        if (commonTypeInfo == null) {
          throw new SemanticException(generateErrorMessage(tabref,
              "Schema of both sides of setop should match: Column " + field + " is of type "
                  + lInfo.getType().getTypeName() + " on first table and type " + rInfo.getType()
      ...
      for (int i = 0; i < leftRowDT.size(); i++) {
        ...
        if (!leftFieldDT.equals(rightFieldDT)) {
          ...
          leftProjs.add(cluster.getRexBuilder()
              .ensureType(unionFieldDT, cluster.getRexBuilder().makeInputRef(leftFieldDT, i),
                  true));

          if (!unionFieldDT.equals(rightFieldDT)) {
            rightNeedsTypeCast = true;
          }
          rightProjs.add(cluster.getRexBuilder()
              .ensureType(unionFieldDT, cluster.getRexBuilder().makeInputRef(rightFieldDT, i),
                  true));
        } else {
          leftProjs.add(cluster.getRexBuilder()
              .ensureType(leftFieldDT, cluster.getRexBuilder().makeInputRef(leftFieldDT, i), true));
          rightProjs.add(cluster.getRexBuilder()
              .ensureType(rightFieldDT, cluster.getRexBuilder().makeInputRef(rightFieldDT, i),
                  true));
        }
      }
      ...
      if (leftNeedsTypeCast) {
        setOpLeftInput = HiveProject.create(leftRel, leftProjs,
            leftRel.getRowType().getFieldNames());
      }
      if (rightNeedsTypeCast) {
        setOpRightInput = HiveProject.create(rightRel, rightProjs,
      ...
      switch (opcode) {
        case UNION:
          ...
          break;
        case INTERSECT:
          setOpRel = new HiveIntersect(cluster, TraitsUtil.getDefaultTraitSet(cluster),
              bldr.build(), false);
          break;
        case INTERSECTALL:
          setOpRel = new HiveIntersect(cluster, TraitsUtil.getDefaultTraitSet(cluster),
    ...
    private RelNode genJoinRelNode(RelNode leftRel, String leftTableAlias, RelNode rightRel,
        String rightTableAlias, JoinType hiveJoinType, ASTNode joinCond) throws SemanticException {
      ...
      if (joinCond != null) {
        ...
        if (joinCond.getType() == HiveParser.TOK_TABCOLNAME && !hiveJoinType.equals(
            JoinType.LEFTSEMI)) {
        ...
        if (jCtx.getError() != null) {
          throw new SemanticException(
              SemanticAnalyzer.generateErrorMessage(jCtx.getErrorSrcNode(), jCtx.getError()));
        }
      ...
      if (leftSemiJoin) {
        ...
        RexNode remainingEquiCond = HiveCalciteUtil.projectNonColumnEquiConditions(
        ...
        calciteJoinCond =
            remainingEquiCond != null ? RexUtil.composeConjunction(cluster.getRexBuilder(),
                ImmutableList.of(remainingEquiCond, nonEquiConds), false) : nonEquiConds;
        ...
        if (inputRels[0] != leftRel) {
          ...
          for (int i = leftRel.getRowType().getFieldCount();
            ...
            ColumnInfo oColInfo = new ColumnInfo(SemanticAnalyzer.getColumnInternalName(i),
          ...
          topRel = HiveRelFactories.HIVE_PROJECT_FACTORY.createProject(topRel, topFields,
              topFieldNames);
        }
     ...
     *
     * @param aliasToRel Alias(Table/Relation alias) to RelNode; only read and not written in to by
     *                   this method
     * @return
     * @throws SemanticException
     */
    private RelNode genJoinLogicalPlan(ASTNode joinParseTree, Map<String, RelNode> aliasToRel)
      ...
      if ((left.getToken().getType() == HiveParser.TOK_TABREF) || (left.getToken().getType()
          == HiveParser.TOK_SUBQUERY) || (left.getToken().getType()
          == HiveParser.TOK_PTBLFUNCTION)) {
      ...
      if ((right.getToken().getType() == HiveParser.TOK_TABREF) || (right.getToken().getType()
          == HiveParser.TOK_SUBQUERY) || (right.getToken().getType()
          == HiveParser.TOK_PTBLFUNCTION)) {
        String tableName = SemanticAnalyzer.getUnescapedUnqualifiedTableName(
      ...
      // 5. Create Join rel
      return genJoinRelNode(leftRel, leftTableAlias, rightRel, rightTableAlias, hiveJoinType,
          joinCond);
    }
    ...
    private RelNode genTableLogicalPlan(String tableAlias, QB qb) throws SemanticException {
      ...
      try {
        ...
        if (qb.getParseInfo().getTabSample(tableAlias) != null
            ...
            || (conf.getBoolVar(HiveConf.ConfVars.HIVE_CBO_RETPATH_HIVEOP)) && (conf.getBoolVar(
            HiveConf.ConfVars.HIVETESTMODE))) {
        ...
        @SuppressWarnings("deprecation") StructObjectInspector rowObjectInspector = (StructObjectInspector) tabMetaData.getDeserializer()
            .getObjectInspector();
        ...
        for (int i = 0; i < fields.size(); i++) {
          ...
          colInfo.setSkewedCol(
              (SemanticAnalyzer.isSkewedCol(tableAlias, qb, colName)) ? true : false);
        ...
        if (tableType == TableType.DRUID || (tableType == TableType.JDBC
            && tabMetaData.getProperty("hive.sql.table") != null)) {
          ...
          for (int i = 0; i < rr.getColumnInfos().size(); i++) {
            cIList.add(
                new ColumnInfo(originalColumnNames.get(i), rr.getColumnInfos().get(i).getType(),
                    tableAlias, false));
          }
          ...
          if (tableType == TableType.DRUID) {
            ...
            JdbcSchema schema = new JdbcSchema(ds, jc.dialect, jc, null/*catalog */,
                null/*schema */);
            ...
            tableRel = new HiveJdbcConverter(cluster,
                jdbcTableRel.getTraitSet().replace(HiveRelNode.CONVENTION), jdbcTableRel, jc);
          }
        } else {
          // Build row type from field <type, name>
          RelDataType rowType = inferNotNullableColumns(tabMetaData,
              TypeConverter.getType(cluster, rr, null));
          // Build RelOptAbstractTable
          String fullyQualifiedTabName = tabMetaData.getDbName();
          if (fullyQualifiedTabName != null && !fullyQualifiedTabName.isEmpty()) {
            fullyQualifiedTabName = fullyQualifiedTabName + "." + tabMetaData.getTableName();
          ...
          tableRel = new HiveTableScan(cluster, cluster.traitSetOf(HiveRelNode.CONVENTION),
    ...
    private RelDataType inferNotNullableColumns(Table tabMetaData, RelDataType rowType)
      ...
      final NotNullConstraint nnc = Hive.get()
          .getReliableNotNullConstraints(tabMetaData.getDbName(), tabMetaData.getTableName());
      // Retrieve primary key constraints (cannot be null)
      final PrimaryKeyInfo pkc = Hive.get()
          .getReliablePrimaryKeys(tabMetaData.getDbName(), tabMetaData.getTableName());
    ...
    private RelNode genFilterRelNode(ASTNode filterExpr, RelNode srcRel,
        ImmutableMap<String, Integer> outerNameToPosMap, RowResolver outerRR, boolean useCaching)
        throws SemanticException {
      ExprNodeDesc filterCondn = genExprNodeDesc(filterExpr, relToHiveRR.get(srcRel), outerRR, null,
          useCaching);
      if (filterCondn instanceof ExprNodeConstantDesc && !filterCondn.getTypeString()
          .equals(serdeConstants.BOOLEAN_TYPE_NAME)) {
      ...
      ImmutableMap<String, Integer> hiveColNameCalcitePosMap = this.relToHiveColNameCalcitePosMap.get(
          srcRel);
      RexNode convertedFilterExpr = new RexNodeConverter(cluster, srcRel.getRowType(),
          outerNameToPosMap, hiveColNameCalcitePosMap, relToHiveRR.get(srcRel), outerRR, 0, true,
          subqueryId).convert(filterCondn);
      RexNode factoredFilterExpr = RexUtil.pullFactors(cluster.getRexBuilder(),
          convertedFilterExpr);
      ...
      for (int i = 0; i < subQueriesInOriginalTree.size(); i++) {
        ...
        if (outerQueryExpr != null && outerQueryExpr.getType() == HiveParser.TOK_SUBQUERY_EXPR) {
          ...
          throw new CalciteSubquerySemanticException(
              ErrorMsg.UNSUPPORTED_SUBQUERY_EXPRESSION.getMsg(outerQueryExpr,
    ...
    private RelNode genLateralViewPlans(ASTNode lateralView, Map<String, RelNode> aliasToRel)
      ...
      final String inlineFunctionName = GenericUDTFInline.class.getAnnotation(Description.class)
          .name();
      ...
      switch (next.getToken().getType()) {
        ...
        case HiveParser.TOK_PTBLFUNCTION:
          ...
          if (next.getToken().getType() == HiveParser.TOK_PTBLFUNCTION) {
            ...
            inputTableAlias = SemanticAnalyzer.unescapeIdentifier(
                next.getChild(1).getText().toLowerCase());
          } else {
            inputTableAlias = next.getChildCount() == 1 ? inputTableName
                : SemanticAnalyzer.unescapeIdentifier(
                    next.getChild(next.getChildCount() - 1).getText().toLowerCase());
          }
      ...
      final ASTNode selExprClause = (ASTNode) lateralView.getChild(0).getChild(0);
      final ASTNode functionCall = (ASTNode) selExprClause.getChild(0);
      ...
      final ASTNode valuesClause = (ASTNode) functionCall.getChild(1);
      ...
      RexCall convertedOriginalValuesExpr = (RexCall) new RexNodeConverter(this.cluster,
          inputRel.getRowType(), inputPosMap, 0, false).convert(valuesExpr);
      ...
      RexNode convertedFinalValuesExpr = rexBuilder.makeCall(convertedOriginalValuesExpr.op,
          newStructExprs);
      // The return type will be the concatenation of input type and original values type
      RelDataType retType = SqlValidatorUtil.deriveJoinRowType(inputRel.getRowType(), valuesRowType,
          JoinRelType.INNER, dtFactory, null, ImmutableList.of());

      // Create inline SQL operator
      FunctionInfo inlineFunctionInfo = FunctionRegistry.getFunctionInfo(inlineFunctionName);
      SqlOperator calciteOp = SqlFunctionConverter.getCalciteOperator(inlineFunctionName,
          inlineFunctionInfo.getGenericUDTF(), ImmutableList.copyOf(inputRefsTypes), retType);

      RelNode htfsRel = HiveTableFunctionScan.create(cluster,
          TraitsUtil.getDefaultTraitSet(cluster), ImmutableList.of(inputRel),
          rexBuilder.makeCall(calciteOp, convertedFinalValuesExpr), null, retType, null);
      ...
      this.relToHiveColNameCalcitePosMap.put(htfsRel,
          buildHiveToCalciteColumnMap(outputRR, htfsRel));
          ...
          case HiveParser.TOK_SUBQUERY_EXPR:
            ...
            if (next.getChildren().size() == 3
              ...
              throw new CalciteSemanticException(
                  ErrorMsg.UNSUPPORTED_SUBQUERY_EXPRESSION.getMsg(next.getChild(2),
    ...
    private RelNode genFilterRelNode(QB qb, ASTNode searchCond, RelNode srcRel,
      ...
      boolean isSubQuery = genSubQueryRelNode(qb, searchCond, srcRel, forHavingClause,
          subQueryToRelNode);
      if (isSubQuery) {
        ExprNodeDesc subQueryExpr = genExprNodeDesc(searchCond, relToHiveRR.get(srcRel), outerRR,
            subQueryToRelNode, forHavingClause);

        ImmutableMap<String, Integer> hiveColNameCalcitePosMap = this.relToHiveColNameCalcitePosMap.get(
            srcRel);
        RexNode convertedFilterLHS = new RexNodeConverter(cluster, srcRel.getRowType(),
            outerNameToPosMap, hiveColNameCalcitePosMap, relToHiveRR.get(srcRel), outerRR, 0, true,
            subqueryId).convert(subQueryExpr);

        RelNode filterRel = new HiveFilter(cluster, cluster.traitSetOf(HiveRelNode.CONVENTION),
            srcRel, convertedFilterLHS);

        this.relToHiveColNameCalcitePosMap.put(filterRel,
    ...
    private AggregateCall convertGBAgg(AggInfo agg, RelNode input, List<RexNode> gbChildProjLst,
      ...
      final SqlAggFunction aggregation = SqlFunctionConverter.getCalciteAggFn(agg.m_udfName,
          agg.m_distinct, aggArgRelDTBldr.build(), aggFnRetType);
    ...
    private RelNode genGBRelNode(List<ExprNodeDesc> gbExprs, List<AggInfo> aggInfoLst,
      ...
      if (hasGroupSets) {
        Set<ImmutableBitSet> setTransformedGroupSets = new HashSet<ImmutableBitSet>(
            groupSets.size());
      ...
      HiveRelNode aggregateRel = new HiveAggregate(cluster,
          cluster.traitSetOf(HiveRelNode.CONVENTION), gbInputRel, groupSet, transformedGroupSets,
          aggregateCalls);
    ...
    private void addAlternateGByKeyMappings(ASTNode gByExpr, ColumnInfo colInfo,
      ...
      if (gByExpr.getType() == HiveParser.DOT
        ...
        String tab_alias = BaseSemanticAnalyzer.unescapeIdentifier(
    ...
    private AggInfo getHiveAggInfo(ASTNode aggAst, int aggFnLstArgIndx, RowResolver inputRR)
      ...
      if (FunctionRegistry.isRankingFunction(aggName)) {
        ...
        try {
          ...
          if (aggName.toLowerCase().equals(FunctionRegistry.LEAD_FUNC_NAME) || aggName.toLowerCase()
              .equals(FunctionRegistry.LAG_FUNC_NAME)) {
            ArrayList<ObjectInspector> originalParameterTypeInfos = SemanticAnalyzer.getWritableObjectInspector(
                aggParameters);
    ...
    private RelNode genGBLogicalPlan(QB qb, RelNode srcRel) throws SemanticException {
      ...
      if (selExprList.getToken().getType() == HiveParser.TOK_SELECTDI && !qb.getAllWindowingSpecs()
          .isEmpty()) {
        return null;
      }
      ...
      boolean hasAggregationTrees =
          (aggregationTrees != null && !aggregationTrees.isEmpty()) ? true : false;
      ...
      if (cubeRollupGrpSetPresent) {
        ...
        if (conf.getBoolVar(HiveConf.ConfVars.HIVEGROUPBYSKEW)) {
          checkExpressionsForGroupingSet(grpByAstExprs,
              qb.getParseInfo().getDistinctFuncExprsForClause(detsClauseName), aggregationTrees,
              this.relToHiveRR.get(srcRel));

          if (qbp.getDestGroupingSets().size() > conf.getIntVar(
      ...
      if (hasGrpByAstExprs || hasAggregationTrees) {
        ...
        if (hasGrpByAstExprs) {
          ...
          for (int i = 0; i < grpByAstExprs.size(); ++i) {
            ...
            Map<ASTNode, ExprNodeDesc> astToExprNDescMap = genAllExprNodeDesc(grpbyExpr,
        ...
        if (hasAggregationTrees) {
          ...
          for (ASTNode value : aggregationTrees.values()) {
            ...
            groupByOutputRowResolver.putExpression(value,
                new ColumnInfo(field, aInfo.m_returnType, "", false));
          }
        }
        ...
        if (groupingSets != null && !groupingSets.isEmpty()) {
          ...
          groupByOutputRowResolver.put(null, VirtualColumn.GROUPINGID.getName(),
              new ColumnInfo(field, VirtualColumn.GROUPINGID.getTypeInfo(), null, true));
        }
    ...
    /**
     * Generate OB RelNode and input Select RelNode that should be used to introduce top
     * constraining Project. If Input select RelNode is not present then don't introduce top
     * constraining select.
     *
     * @param qb
     * @param selPair
     ...
     * @throws SemanticException
     */
    private RelNode genOBLogicalPlan(QB qb, Pair<RelNode, RowResolver> selPair, boolean outermostOB)
        throws SemanticException {
      ...
      if (obAST != null) {
        ...
        for (int i = 0; i < obASTExprLst.size(); i++) {
          ...
          boolean isObyByPos =
              isBothByPos || HiveConf.getBoolVar(conf, ConfVars.HIVE_ORDERBY_POSITION_ALIAS);
              ...
              if (pos > 0 && pos <= selectOutputRR.getColumnInfos().size()) {
                ...
                throw new SemanticException(ErrorMsg.INVALID_POSITION_ALIAS_IN_ORDERBY.getMsg(
                    "Position alias: " + pos + " does not exist\n"
                        + "The Select List is indexed from 1 to " + selectOutputRR.getColumnInfos()
                        .size()));
              }
            } else { // if not using position alias and it is a number.
              LOG.warn("Using constant number " + ref.getText()
        ...
        if (!newVCLst.isEmpty()) {
          ...
          for (Pair<ASTNode, TypeInfo> astTypePair : vcASTTypePairs) {
            obSyntheticProjectRR.putExpression(astTypePair.getKey(),
                new ColumnInfo(SemanticAnalyzer.getColumnInternalName(vcolPos),
                    astTypePair.getValue(), null, false));
            vcolPos++;
          }
    ...
    private RelNode genLimitLogicalPlan(QB qb, RelNode srcRel) throws SemanticException {
      ...
      QBParseInfo qbp = getQBParseInfo(qb);
      SimpleEntry<Integer, Integer> entry = qbp.getDestToLimit()
          .get(qbp.getClauseNames().iterator().next());
      ...
      if (fetch != null) {
        ...
        sortRel = new HiveSortLimit(cluster, traitSet, srcRel, canonizedCollation, offsetRN,
            fetchRN);
    ...
    private RexWindowBound getBound(BoundarySpec bs, RexNodeConverter converter) {
      ...
      if (bs != null) {
        ...
        SqlNode amt = bs.getAmt() == 0 ? null
            : SqlLiteral.createExactNumeric(String.valueOf(bs.getAmt()), new SqlParserPos(2, 2));
        RexNode amtLiteral = null;
        SqlCall sc = null;

        if (amt != null) {
          amtLiteral = cluster.getRexBuilder()
              .makeLiteral(bs.getAmt(), cluster.getTypeFactory().createSqlType(SqlTypeName.INTEGER),
                  true);
        }
        ...
        switch (bs.getDirection()) {
          ...
          case FOLLOWING:
            if (amt == null) {
              rwb = RexWindowBound.create(
                  SqlWindow.createUnboundedFollowing(new SqlParserPos(1, 1)), null);
            } else {
    ...
    private Pair<RexNode, TypeInfo> genWindowingProj(QB qb, WindowExpressionSpec wExpSpec,
      ...
      if (wExpSpec instanceof WindowFunctionSpec) {
        ...
        for (int i = 0; i < hiveAggInfo.m_aggParams.size(); i++) {
          ...
          calciteAggFnArgsTypeBldr.add(
              TypeConverter.convert(hiveAggInfo.m_aggParams.get(i).getTypeInfo(),
                  this.cluster.getTypeFactory()));
        }
        ...
        final SqlAggFunction calciteAggFn = SqlFunctionConverter.getCalciteAggFn(
            hiveAggInfo.m_udfName, hiveAggInfo.m_distinct, calciteAggFnArgsType,
            calciteAggFnRetType);
        ...
        w = cluster.getRexBuilder()
            .makeOver(calciteAggFnRetType, calciteAggFn, calciteAggFnArgs, partitionKeys,
                ImmutableList.<RexFieldCollation>copyOf(orderKeys), lowerBound, upperBound, isRows,
                true, false, hiveAggInfo.m_distinct);
      } else {
    ...
    private RelNode genSelectForWindowing(QB qb, RelNode srcRel, HashSet<ColumnInfo> newColumns)
      ...
      getQBParseInfo(qb);
      WindowingSpec wSpec =
          (!qb.getAllWindowingSpecs().isEmpty()) ? qb.getAllWindowingSpecs().values().iterator()
              .next() : null;
      if (wSpec == null) {
        return null;
      }
      ...
      for (WindowExpressionSpec wExprSpec : windowExpressions) {
        if (!qbp.getDestToGroupBy().isEmpty()) {
          ...
          wExprSpec.setExpression(
              rewriteGroupingFunctionAST(getGroupByForClause(qbp, selClauseName),
                  wExprSpec.getExpression(), !cubeRollupGrpSetPresent));
        }
    ...
    private RelNode genSelectRelNode(List<RexNode> calciteColLst, RowResolver out_rwsch,
      ...
      this.relToHiveColNameCalcitePosMap.put(selRel,
          buildHiveToCalciteColumnMap(out_rwsch, selRel));
    ...
    private void setQueryHints(QB qb) throws SemanticException {
      ...
      String hint = ctx.getTokenRewriteStream()
          .toString(selExpr0.getTokenStartIndex(), selExpr0.getTokenStopIndex());
     ...
     * @param outerRR
     * @param isAllColRefRewrite when it is true, it means that it is called from group by *, where
     *                           we use genSelectLogicalPlan to rewrite *
     * @return RelNode: the select relnode RowResolver: i.e., originalRR, the RR after select when
     * there is an order by.
     * @throws SemanticException
     */
    private Pair<RelNode, RowResolver> genSelectLogicalPlan(QB qb, RelNode srcRel,
        RelNode starSrcRel, ImmutableMap<String, Integer> outerNameToPosMap, RowResolver outerRR,
      ...
      if (exprType == HiveParser.TOK_FUNCTION || exprType == HiveParser.TOK_FUNCTIONSTAR) {
        ...
        if (fi != null && fi.getGenericUDTF() != null) {
          ...
          if (genericUDTF != null && (selectStar = exprType == HiveParser.TOK_FUNCTIONSTAR)) {
            genColListRegex(".*", null, (ASTNode) expr.getChild(0), col_list, null, inputRR, starRR,
                pos, out_rwsch, qb.getAliases(), false);
          }
        }
      }
      ...
      for (int i = startPosn; i < exprList.getChildCount(); ++i) {
        ...
        if (genericUDTF == null && child.getChildCount() > 2) {
          throw new SemanticException(
              SemanticAnalyzer.generateErrorMessage((ASTNode) child.getChild(2),
        ...
        if (isSubQuery) {
          ExprNodeDesc subQueryExpr = genExprNodeDesc(expr, relToHiveRR.get(srcRel), outerRR,
              subQueryToRelNode, true);
          ...
          if (!out_rwsch.putWithCheck(tabAlias, colAlias, null, colInfo)) {
            throw new CalciteSemanticException(
                "Cannot add column to RR: " + tabAlias + "." + colAlias + " => " + colInfo
          ...
          if (expr.getType() == HiveParser.TOK_ALLCOLREF) {
            pos = genColListRegex(".*", expr.getChildCount() == 0 ? null
                    : SemanticAnalyzer.getUnescapedName((ASTNode) expr.getChild(0)).toLowerCase(), expr,
                col_list, excludedColumns, inputRR, starRR, pos, out_rwsch, qb.getAliases(), true);
            selectStar = true;
          } else if (expr.getType() == HiveParser.TOK_TABLE_OR_COL && !hasAsClause
              && !inputRR.getIsExprResolver() && SemanticAnalyzer.isRegex(
            ...
            pos = genColListRegex(SemanticAnalyzer.unescapeIdentifier(expr.getChild(0).getText()),
              ...
              && expr.getChild(0).getType() == HiveParser.TOK_TABLE_OR_COL && inputRR.hasTableAlias(
              SemanticAnalyzer.unescapeIdentifier(
                  expr.getChild(0).getChild(0).getText().toLowerCase())) && !hasAsClause
              && !inputRR.getIsExprResolver() && SemanticAnalyzer.isRegex(
              SemanticAnalyzer.unescapeIdentifier(expr.getChild(1).getText()), conf)) {
            // In case the expression is TABLE.COL (col can be regex).
            // This can only happen without AS clause
            // We don't allow this for ExprResolver - the Group By case
            pos = genColListRegex(SemanticAnalyzer.unescapeIdentifier(expr.getChild(1).getText()),
                SemanticAnalyzer.unescapeIdentifier(
                    expr.getChild(0).getChild(0).getText().toLowerCase()), expr, col_list,
            ...
            colInfo.setSkewedCol(
                (exp instanceof ExprNodeColumnDesc) ? ((ExprNodeColumnDesc) exp).isSkewedCol()
                    : false);
            if (!out_rwsch.putWithCheck(tabAlias, colAlias, null, colInfo)) {
              throw new CalciteSemanticException(
                  "Cannot add column to RR: " + tabAlias + "." + colAlias + " => " + colInfo
      ...
      RexNodeConverter rexNodeConv = new RexNodeConverter(cluster, srcRel.getRowType(),
          outerNameToPosMap, buildHiveColNameToInputPosMap(col_list, inputRR),
          relToHiveRR.get(srcRel), outerRR, 0, false, subqueryId);
      ...
      if (genericUDTF != null) {
        ...
        if (obAST != null && !(selForWindow != null
          ...
          for (int i = 0; i < inputRR.getColumnInfos().size(); i++) {
            ...
            String internalName = SemanticAnalyzer.getColumnInternalName(
                out_rwsch.getColumnInfos().size() + i);
      ...
      if (selForWindow != null && selExprList.getToken().getType() == HiveParser.TOK_SELECTDI) {
        ImmutableBitSet groupSet = ImmutableBitSet.range(
            outputRel.getRowType().getFieldList().size());
        ...
        for (int i = 0; i < out_rwsch.getColumnInfos().size(); i++) {
          ...
          ColumnInfo newColInfo = new ColumnInfo(colInfo.getInternalName(), colInfo.getType(),
    ...
    private RelNode genUDTFPlan(GenericUDTF genericUDTF, String genericUDTFName,
        String outputTableAlias, ArrayList<String> colAliases, QB qb, List<RexNode> selectColLst,
      ...
      StandardStructObjectInspector rowOI = ObjectInspectorFactory.getStandardStructObjectInspector(
          colNames, Arrays.asList(colOIs));
    ...
    private RelNode genLogicalPlan(QBExpr qbexpr) throws SemanticException {
      switch (qbexpr.getOpcode()) {
        ...
        case EXCEPTALL:
          ...
          return genSetOpLogicalPlan(qbexpr.getOpcode(), qbexpr.getAlias(),
              qbexpr.getQBExpr1().getAlias(), qbexpr1Ops, qbexpr.getQBExpr2().getAlias(),
              qbexpr2Ops);
        default:
          return null;
      }
    ...
    private RelNode genLogicalPlan(QB qb, boolean outerMostQB,
      ...
      for (String subqAlias : qb.getSubqAliases()) {
        ...
        if (qb.getViewToTabSchema().containsKey(subqAlias)) {
          if (relNode instanceof HiveProject) {
            ...
            throw new SemanticException(
                "View " + subqAlias + " is corresponding to " + relNode.toString()
                    + ", rather than a HiveProject.");
          }
        }
      }
      ...
      if (qb.getParseInfo().getJoinExpr() != null) {
        ...
        // If it contains a LV
        List<ASTNode> lateralViews = getQBParseInfo(qb).getAliasToLateralViews()
            .get(uniqueAliasToRel.getKey());
      ...
      Pair<RelNode, RowResolver> selPair = genSelectLogicalPlan(qb, srcRel, starSrcRel,
          outerNameToPosMap, outerRR, false);
      ...
      if (havingClause != null) {
        ...
        if (!qbp.getDestToGroupBy().isEmpty()) {
          ...
          targetNode = rewriteGroupingFunctionAST(getGroupByForClause(qbp, destClauseName),
              targetNode, !cubeRollupGrpSetPresent);
        }
    ...
    private void validateNoHavingReferenceToAlias(QB qb, ASTNode havingExpr)
      ...
      for (Map.Entry<ASTNode, String> selExpr : exprToAlias.entrySet()) {
        ...
        if (aliasReferences.size() > 0) {
          String havingClause = ctx.getTokenRewriteStream()
              .toString(havingExpr.getTokenStartIndex(), havingExpr.getTokenStopIndex());
    ...
    private ImmutableMap<String, Integer> buildHiveColNameToInputPosMap(List<ExprNodeDesc> col_list,
        RowResolver inputRR) {
